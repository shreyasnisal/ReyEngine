...
UINT GetWindowsMessageBoxIconFlagForSeverityLevel( MsgSeverityLevel severity )
{
	switch( severity )
	{
		case MsgSeverityLevel::INFORMATION:		return MB_ICONASTERISK;	
		case MsgSeverityLevel::QUESTION:		return MB_ICONQUESTION;
		case MsgSeverityLevel::WARNING:			return MB_ICONEXCLAMATION;
		case MsgSeverityLevel::FATAL:			return MB_ICONHAND;	
...
char const* FindStartOfFileNameWithinFilePath( char const* filePath )
{
	...
	char const* scan = filePath + pathLen;
	while( scan > filePath )
	{
		-- scan;
...
__declspec( noreturn ) void FatalError( char const* filePath, char const* functionName, int lineNum, std::string const& reasonForError, char const* conditionText )
{
	...
	std::string appName = g_gameConfigBlackboard.GetValue("gameTitle", "Unnamed Application");

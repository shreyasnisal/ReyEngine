...
//-----------------------------------------------------------------------------------------------
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath, bool is3DSound )
{
	...
	else
	{
		...
		FMOD_RESULT fmodResult = m_fmodSystem->createSound( soundFilePath.c_str(), FMOD_DEFAULT | (is3DSound ? FMOD_3D : 0), nullptr, &newSound );
		if( fmodResult == FMOD_OK && newSound )
		{
...
void AudioSystem::UpdateListeners(int listenerIndex, Vec3 const& listenerPosition, Vec3 const& listenerForward, Vec3 const& listenerUp)
{
	FMOD_VECTOR fmodListenerPosition = GetFModVectorFromVec3_iFwd_jLeft_kUp(listenerPosition);
	FMOD_VECTOR fmodListenerVelocity{0.f, 0.f, 0.f};
	FMOD_VECTOR fmodListenerFwd = GetFModVectorFromVec3_iFwd_jLeft_kUp(listenerForward);
	FMOD_VECTOR fModListenerUp = GetFModVectorFromVec3_iFwd_jLeft_kUp(listenerUp);

	m_fmodSystem->set3DListenerAttributes(listenerIndex, &fmodListenerPosition, &fmodListenerVelocity, &fmodListenerFwd, &fModListenerUp);
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, Vec3 const& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_VECTOR fmodSoundPosition = GetFModVectorFromVec3_iFwd_jLeft_kUp(soundPosition);
		FMOD_VECTOR fmodSoundVelocity{0.f, 0.f, 0.f};
		channelAssignedToSound->set3DAttributes(&fmodSoundPosition, &fmodSoundVelocity);
	}
...
FMOD_VECTOR const AudioSystem::GetFModVectorFromVec3_iFwd_jLeft_kUp(Vec3 const& vec3)
{
